...
| 10 | `codex/10_CONTROLLED_GITLAB_WRITEBACK.md` | `codex/gitlab-controlled-writeback` | approved GitLab comment writeback |
...
- Task 10 is the first task allowed to add a real external write route, and it must verify target provenance before calling any GitLab write client.
